...
> например, ~90–132 запроса и p95 ~25–35 с в разных запусках. **Доля ошибок стабильно 0 %**,
> оба порога проходят всегда. Цифры в таблице — один из представительных прогонов.

> **Стабильность под нагрузкой (находка).** Один прогон дал ~100% ошибок `HTTP 500
> "Cannot connect to ollama:11434"`: на узле 2 vCPU / 4 ГБ, где Ollama соседствовал с LiteLLM и
> Grafana, под устойчивой нагрузкой возникало memory pressure → kubelet **вытеснял Ollama-под**,
> и пока тот холодно стартовал (перезагрузка моделей), весь трафик падал. Лечится без увеличения
> железа: подняли Ollama `requests.memory` до 2Gi (перестал быть первым кандидатом на eviction)
> и добавили soft `podAntiAffinity`, чтобы Ollama не делил узел с LiteLLM/Grafana
> (`helm/ollama-values.yaml`). После этого прогон снова 0 % ошибок, throughput даже выше
> (Ollama получил отдельный узел) — Ollama не вытеснялся.
...
**Скриншоты** (`docs/screenshots/`): обе модели в выпадашке OpenWebUI, рабочий чат, итог k6 в
терминале и дашборд **LLM Stand** на пике нагрузки в трёх разрезах — ресурсы подов, HTTP-метрики
LLM API (ingress), app-метрики LiteLLM по моделям. Наглядно собрано в
[`docs/walkthrough.md`](docs/walkthrough.md).
